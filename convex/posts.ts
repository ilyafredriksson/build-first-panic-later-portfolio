...
export const getPosts = query({
  ...
  handler: async (ctx) => {
    ...
    return await Promise.all(posts.map(async (post)=>{
        const resolvedImagaeUrl = post.imageStorageId !== undefined 
        ? await ctx.storage.getUrl(post.imageStorageId) :null
        return {
            ...post,
            imageUrl: resolvedImagaeUrl,
        };
    }));
  },
});
...
export const getPostById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const reslvedImageUrl= post? post.imageStorageId !== undefined ? await ctx.storage.getUrl(post.imageStorageId) : null : null;
       
        return {
            ...post,
            imageUrl: reslvedImageUrl,
        }
